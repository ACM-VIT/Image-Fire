...
'''
vs = cv2.VideoCapture('videoCUT.mp4')
writer = None
(W, H) = (None, None)
...
while True:
    ...
    if len(idxs) > 0:
        ...
        if park == 1:
            t = "parked"
            db.child("ParkingStatus").update({"State": "Yes"})
        else:
            t = "not parked"
            db.child("ParkingStatus").update({"State": "No"})
        cv2.putText(frame, t, (8, 16), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (25, 0, 0), 2)
        '''
